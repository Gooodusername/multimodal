...
# DISFA AU9 AU25 AU26
def save_au(pathes, aues, pretrained_type='DISFA'):
    for i in range(len(pathes)):
        ...
        # save_path = './data/extracted_au/%s/%s/%s/%s.pth'%(data_type, group, pid, img_id)
        save_path = './data/extracted_au/%s/%s/%s.pth' % (group, pid, img_id)
        save_path_spilt = save_path.split('/')

        for index, each_save_path in enumerate(save_path_spilt):
            if index == 0:
                new_save_path = "./"
                continue
            else:
                 if '.' in each_save_path:
                     break
                 if not(each_save_path in os.listdir(new_save_path)):
                     os.mkdir("%s/%s"%(new_save_path, each_save_path))
                 new_save_path = "%s/%s"%(new_save_path, each_save_path)
...
if __name__=="__main__":
    ...
    extract(128, 224, r'C:\Users\26804\Desktop\extract\extract\ME-GraphAU\checkpoints\MEFARG_swin_base_DISFA_fold1.pth', 'DISFA')
    # extract(128, 224, r'C:\Users\26804\Desktop\extract\extract\ME-GraphAU\checkpoints\MEFARG_swin_base_BP4D_fold1.pth', 'BP4D')
